<template>
  ...
  const gmSystemEmailStore = useGMSystemEmailStore()
  
  // 使用store中的状态
  const { 
    systemEmailList: tableData, 
    loading, 
    total, 
    page, 
    pageSize, 
    ...
    resourceList,
    resourceMap
  } = storeToRefs(gmSystemEmailStore)
  ...
  const handleSizeChange = (val) => {
    ...
    // 使用多语言 Composable
    const { activeSenderTab, activeTitleTab, activeContentTab, resetActiveTabs, setActiveTabsFromData } = useMultilingual()
